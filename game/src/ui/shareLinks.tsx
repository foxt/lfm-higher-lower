...
export function ShareLinks({text}: {text: string}) {
    ...
    return <div class='slinks'>
        <a class='s-link twt' href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(location.href)}`} target='_blank' rel='noopener noreferrer'>
            <img src={TwitterLogo} alt='Twitter Logo' height={32} />
        </a>
        <a class='s-link msto' onClick={doMastoShare} target='_blank' rel='noopener noreferrer'>
            <img src={MastoLogo} alt='Mastodon Logo' height={32} />
        </a>
        <a class='s-link tg' href={`https://t.me/share/url?url=${encodeURIComponent(location.href)}&text=${encodeURIComponent(text)}`} target='_blank' rel='noopener noreferrer'>
            <img src={TelegramLogo} alt='Telegram Logo' height={32} />
        </a>
        {"share" in navigator && <button class='s-link native' onClick={doNativeShare}>
